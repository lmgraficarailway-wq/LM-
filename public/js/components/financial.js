        ...
        if (sortedKeys.length === 0) {
            ...
            monthlyContainer.innerHTML = sortedKeys.map(key => {
                ...
                <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(170px,1fr)); gap:1rem; margin-bottom:1.5rem;">
                    <!-- Transações — verde escuro saturado -->
                    <div style="background:linear-gradient(135deg,#bbf7d0,#86efac); border:1.5px solid #4ade80; border-radius:14px; padding:1.1rem 1.25rem; display:flex; align-items:center; gap:0.9rem; box-shadow:0 2px 8px rgba(21,128,61,0.18); transition:transform 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='none'">
                        <div style="width:40px; height:40px; border-radius:10px; background:linear-gradient(135deg,#15803d,#14532d); display:flex; align-items:center; justify-content:center; color:white; font-size:1.1rem; flex-shrink:0; box-shadow:0 3px 10px rgba(21,128,61,0.4);">🧾</div>
                        <div>
                            <div style="font-size:1.5rem; font-weight:900; color:#14532d; line-height:1;">${m.sales.length}</div>
                            <div style="font-size:0.75rem; color:#166534; font-weight:700; text-transform:uppercase; letter-spacing:0.04em; margin-top:2px;">Transações</div>
                        </div>
                    </div>
